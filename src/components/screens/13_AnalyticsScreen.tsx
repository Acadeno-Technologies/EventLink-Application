...
export const AnalyticsScreen: React.FC = () => {
  ...
  const sourceData = totalRegs > 0 ? [
    { name: 'WhatsApp Shares', value: whatsappCount, color: '#10B981' },
    { name: 'Direct Link', value: directCount, color: '#2563EB' },
    { name: 'QR Code Scan', value: qrCount, color: '#06B6D4' },
    { name: 'Social / UTM', value: socialCount, color: '#F59E0B' },
  ].filter(s => s.value > 0) : [
    ...
    <AdminLayout
      ...
      headerAction={
        <button
          ...
          className="h-9 px-3.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition-colors cursor-pointer"
        >
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-xs">
          ...
          <p className="text-[11px] text-slate-400 mt-1">Completed attendee submissions</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-xs">
          ...
          <div className="text-3xl font-extrabold text-amber-600 font-display">{attendanceRate}%</div>
        ...
        <div className="lg:col-span-8 bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs flex flex-col justify-between">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Timeline of incoming registrations leading up to event start</p>
            </div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={dailyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#0B172B', borderRadius: '12px', border: 'none', color: '#fff', fontSize: '11px', boxShadow: '0 10px 25px rgba(0,0,0,0.2)' }}
                />
                <Bar dataKey="views" fill="#E2E8F0" radius={[6, 6, 0, 0]} name="Page Views" />
                <Bar dataKey="registrations" fill="#2563EB" radius={[6, 6, 0, 0]} name="Registrations" />
              </BarChart>
            </ResponsiveContainer>
          </div>

          <div className="flex items-center justify-between text-xs text-slate-400 pt-3 border-t border-slate-100">
            <span>Aggregated across live form submissions</span>
            <span className="text-slate-700 font-bold">8-Day Activity Window</span>
          </div>
        </div>

        {/* Right 4 Cols: Source Breakdown Pie */}
        <div className="lg:col-span-4 bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs space-y-6 flex flex-col justify-between">
          <div>
            <div className="border-b border-slate-100 pb-3 mb-4">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700">Traffic Source Breakdown</h3>
              <p className="text-xs text-slate-500 mt-0.5">Captured at participant form submission</p>
            </div>
            ...
            <div className="h-44 w-full flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <RechartsPie>
                  ...
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#0B172B', borderRadius: '8px', border: 'none', color: '#fff', fontSize: '11px' }}
                  />
                </RechartsPie>
              </ResponsiveContainer>
            </div>
            ...
            <div className="space-y-2 pt-2">
              {sourceData.map((s, idx) => (
                <div key={idx} className="p-2.5 rounded-xl bg-slate-50/80 border border-slate-100 flex items-center justify-between text-xs">
          ...
          <div className="p-3.5 bg-blue-50/70 rounded-xl border border-blue-100 text-xs text-blue-900 leading-relaxed font-medium">
            💡 <strong>Channel insight:</strong> Direct link and WhatsApp shares generate the highest participant conversion velocity.
          </div>
        </div>
