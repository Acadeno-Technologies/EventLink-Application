...
export const EventsListScreen: React.FC = () => {
  ...
  return (
    <AdminLayout
      activeNav="events"
      pageTitle="Events"
      pageSubtitle="Browse, organize, and monitor all active, draft, and archived event pages."
      headerAction={
        <button
          onClick={startNewEventWizard}
          className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 px-4 rounded-xl flex items-center gap-2 shadow-sm transition-all text-xs group"
        >
          <Plus className="w-4 h-4 text-orange-300 group-hover:rotate-90 transition-transform" />
    ...
    >
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:bg-white"
            />
          </div>
          ...
          return (
            ...
            >
              ...
              <div className="relative h-44 bg-slate-100 overflow-hidden">
                ...
                {/* Status Badges per spec: Live = green, Closed = rose, Draft = gray/blue, Archived = muted */}
                <div className="absolute top-3 left-3">
                  <span className={`text-[11px] font-bold px-3 py-1 rounded-full shadow-xs ${
                    evt.status === 'active'
                      ...
                      : evt.status === 'draft'
                      ? 'bg-blue-900/80 text-blue-200 backdrop-blur-xs'
                ...
                <div className="absolute top-3 right-3">
                  <div className="relative">
                    <button
                      ...
                      className="p-1.5 bg-slate-900/70 hover:bg-slate-900 text-white rounded-lg backdrop-blur-sm transition-colors cursor-pointer"
                    >
                ...
                <div className="absolute bottom-3 left-3 right-3 text-white">
                  <h3 
                    ...
                    className="font-bold text-base line-clamp-1 cursor-pointer hover:text-cyan-300 transition-colors"
                  >
                    {evt.name}
                  </h3>
                </div>
      ...
      {events.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
          ...
          <h3 className="text-base font-bold text-slate-800 font-display">No events in database yet</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1 mb-5">
            Your database is fresh and ready. Create your first event using the 5-step wizard to start managing registrations.
          </p>
          <button
            onClick={startNewEventWizard}
            className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs py-2.5 px-5 rounded-xl shadow-sm transition-all"
          >
        ...
        <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 p-8">
          ...
          <h3 className="text-base font-bold text-slate-800">No events match filter</h3>
