    ...
    <AdminLayout activeNav="events">
      <div className="space-y-4 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3.5 items-start">
          ...
          {/* ========================================================================= */}
          {/* LEFT COLUMN: 3 DISTINCT COMPACT CARDS (lg:col-span-7)                     */}
          {/* ========================================================================= */}
          <div className="lg:col-span-7 space-y-3">

            {/* CARD 1 — CUSTOM COLORS & BRANDING */}
            <div className="bg-white rounded-xl border border-[#DCE5F0] p-3.5 shadow-2xs space-y-2">
              <div className="flex items-center gap-2 border-b border-slate-100 pb-1.5">
                <Palette className="w-3.5 h-3.5 text-[#1463FF]" />
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                ...
                <div className="space-y-0.5">
                  ...
                  <div className="flex items-center justify-between h-8 px-2 bg-white border border-[#DCE5F0] rounded-lg focus-within:border-[#1463FF] focus-within:ring-2 focus-within:ring-[#1463FF]/10 transition-all cursor-pointer">
                    <div className="flex items-center min-w-0 mr-1 flex-1">
                      <input
                        type="color"
                        value={currentTheme.colors.primary}
                        onChange={(e) => handleUpdateColors('primary', e.target.value)}
                        className="w-4 h-4 rounded-md cursor-pointer border-0 p-0 mr-1.5 bg-transparent shrink-0"
                      />
                      <input
                        type="text"
                        value={currentTheme.colors.primary}
            ...
            <div className="bg-white rounded-xl border border-[#DCE5F0] p-3.5 shadow-2xs space-y-2">
              <div className="flex items-center justify-between border-b border-slate-100 pb-1.5">
                ...
                >
                  ...
                  <span>{showUrlInput ? 'File Upload' : 'Paste Image URL'}</span>
                </button>
              </div>
              ...
              {showUrlInput ? (
                <div className="space-y-1.5">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full h-8 pl-8.5 pr-3 bg-white border border-[#DCE5F0] rounded-lg text-xs text-slate-900 placeholder-[#91A4C0] focus:outline-none focus:ring-2 focus:ring-[#1463FF]/10 focus:border-[#1463FF] font-medium"
                    />
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-2.5 items-center">
                  ...
                  {/* Upload Box */}
                  <label className="sm:col-span-8 bg-[#F8FAFC] hover:bg-[#F1F5F9] border-2 border-dashed border-[#CBD5E1] hover:border-[#1463FF] rounded-xl px-2.5 py-1.5 flex items-center justify-between gap-2.5 cursor-pointer transition-all h-[46px]">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className="w-7 h-7 rounded-lg bg-[#EFF6FF] text-[#1463FF] flex items-center justify-center shrink-0 border border-blue-100">
                        {isUploadingBanner ? (
                    ...
                    <div className="bg-white hover:bg-slate-50 text-slate-700 border border-[#DCE5F0] text-[11px] font-semibold px-2 py-0.5 rounded-md transition-all shrink-0 shadow-2xs">
                      Browse
                    </div>
                    <input
                      type="file"
                  ...
                  {/* Banner Thumbnail */}
                  <div className="sm:col-span-4 relative h-[46px] rounded-xl overflow-hidden border border-[#DCE5F0] shadow-2xs bg-slate-900 group">
                    <img
                      src={currentBanner}
                      alt="Banner preview"
                      onError={(e) => {
                    ...
                    <button
                      ...
                      className="w-4.5 h-4.5 bg-white/95 hover:bg-white text-slate-600 hover:text-rose-600 rounded-full flex items-center justify-center shadow-xs border border-slate-200 absolute top-1 right-1 transition-colors cursor-pointer"
                      title="Delete banner"
                    >
                ...
                {bannerPresets.map((preset, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`text-[11px] font-semibold px-2 py-0.5 rounded-md border transition-all shrink-0 cursor-pointer ${isSelected
            ...
            {/* CARD 3 — TYPOGRAPHY & FONT STYLE (COMPACT, ZERO SCROLLBAR) */}
            <div className="bg-white rounded-xl border border-[#DCE5F0] p-3.5 shadow-2xs space-y-2">
              <div className="flex items-center justify-between border-b border-slate-100 pb-1.5">
                <div className="flex items-center gap-2">
                  <Type className="w-3.5 h-3.5 text-[#1463FF]" />
                  <h3 className="text-xs font-bold uppercase tracking-wider text-[#101B33]">
                    Typography & Font Style
                  </h3>
                </div>

                {/* Font Selector Dropdown for all 16+ Google Fonts */}
                <div className="flex items-center gap-1.5">
                  <select
                    value={currentTheme.typography.fontFamily}
                    onChange={(e) => handleUpdateFont(e.target.value)}
                    className="h-7 pl-2 pr-6 bg-[#F8FAFC] hover:bg-white border border-[#DCE5F0] rounded-lg text-[11px] font-semibold text-slate-800 focus:outline-none focus:border-[#1463FF] focus:ring-1 focus:ring-[#1463FF]/20 transition-all cursor-pointer"
                  >
              ...
              {/* 4 Popular Font Cards (Single Row, No Scrollbar) */}
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {allFontOptions.slice(0, 4).map((font) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-2 rounded-xl border text-left transition-all cursor-pointer flex items-center justify-between gap-1.5 ${isSelected
                    ...
                    >
                      <div className="min-w-0">
                        <div
                          className="text-xs font-bold text-[#101B33] truncate"
                          style={{ fontFamily: font.id }}
                        >
                          {font.label}
                        </div>
                        <div className="text-[9.5px] text-[#7184A3] font-medium truncate">
                          {font.desc}
                        </div>
                      </div>

                      <div className={`w-3 h-3 rounded-full border flex items-center justify-center shrink-0 transition-colors ${isSelected
          ...
          {/* ========================================================================= */}
          <div className="lg:col-span-5 bg-white rounded-xl border border-[#DCE5F0] p-3.5 shadow-2xs flex flex-col items-center justify-between">

            {/* Card Header Tag */}
            <div className="w-full flex items-center justify-between border-b border-slate-100 pb-1.5 mb-2">
              <div className="flex items-center gap-1.5">
            ...
            {/* Compact Smartphone Frame Mockup */}
            <div className="w-full max-w-[270px] sm:max-w-[285px] rounded-[26px] p-2 bg-[#0B1528] border-2 border-slate-800 shadow-lg relative my-auto">

              {/* Phone Speaker & Camera Notch */}
              <div className="w-16 h-1.5 bg-slate-800 rounded-b-md mx-auto mb-1.5 flex items-center justify-center gap-1">
              ...
              <div
                className="rounded-[18px] overflow-hidden p-1.5 flex flex-col justify-start shadow-inner transition-colors duration-300 relative"
                style={{
              ...
              >
                ...
                <div className="w-full bg-white rounded-lg shadow-sm overflow-hidden border border-black/5 flex flex-col transition-all">

                  {/* Event Banner Image */}
                  <div className="w-full h-16 relative overflow-hidden bg-slate-950 shrink-0">
                    <img
                      src={currentBanner}
                      alt="Event banner"
                      onError={(e) => {
                  ...
                  <div
                    className="px-2.5 py-1.5 text-white relative transition-colors duration-300"
                    style={{ backgroundColor: currentTheme.colors.primary || '#2563EB' }}
                  >
                    <h4 className="text-xs font-extrabold leading-tight mb-0.5 truncate">
                      {wizardDraft.name || 'ACADENO Event'}
                    </h4>

                    <div className="space-y-0.5 text-white/90 text-[10px] font-medium">
                      <div className="flex items-center gap-1">
                        <Calendar className="w-2.5 h-2.5 shrink-0" />
                        <span className="truncate">{wizardDraft.start_date || '2026-09-11'} | {wizardDraft.start_time || '10:00 AM'}</span>
                      </div>
                      <div className="flex items-center gap-1">
                  ...
                  <div
                    className="p-2.5 flex-1 flex flex-col justify-between space-y-1.5 bg-white"
                  ...
                  >
                    <div className="space-y-1.5">
                      ...
                      <div className="space-y-1.5">
                        <div>
                          <label
                            className="block text-[10px] font-bold mb-0.5 transition-colors"
                            style={{ color: currentTheme.colors.text || '#101B33' }}
                          >
                    ...
                    <div className="pt-1 space-y-1">
                      <button
                        ...
                        className="w-full h-7 rounded-lg font-bold text-[11px] shadow-2xs flex items-center justify-center gap-1 transition-transform active:scale-95 cursor-pointer"
                      >
                      ...
                      <div
                        className="text-[9px] text-center font-medium transition-colors"
        ...
        <div className="flex items-center justify-between pt-1">
          <button
            ...
            className="h-9.5 px-4 rounded-xl border border-[#DCE5F0] bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer shadow-2xs"
          >
          ...
          <button
            ...
            className="h-9.5 px-6 rounded-xl bg-[#1463FF] hover:bg-[#0E4ED8] text-white text-xs font-bold shadow-[0_3px_12px_rgba(20,99,255,0.25)] flex items-center gap-1.5 transition-all cursor-pointer"
          >
