    ...
    return (
      ...
      >
        <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
          <div className="w-14 h-14 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center mx-auto mb-3 border border-indigo-100 shadow-2xs">
  ...
  return (
    <AdminLayout
      ...
      headerAction={
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handlePrint}
            className="px-3.5 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold transition-colors flex items-center gap-1.5 shadow-sm"
          >
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 p-6 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              <div className="p-3.5 rounded-lg bg-slate-50 border border-slate-100 space-y-1">
                ...
                <div className="text-sm font-semibold text-slate-800 flex items-center gap-1.5 font-mono">
                  <Phone className="w-3.5 h-3.5 text-slate-400" />
                  <span>{getPhoneDisplay(reg)}</span>
                </div>
              </div>
            ...
            <div className="pt-2 border-t border-slate-100 space-y-3">
              ...
              <div className="divide-y divide-slate-100 rounded-lg border border-slate-200/80 overflow-hidden bg-slate-50/50">
                {selectedEvent?.form_schema ? (
                  selectedEvent.form_schema.map((f) => {
                    const answer = reg.responses?.[f.id];
                    return (
                      <div key={f.id} className="p-3 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                  ...
                  Object.entries(reg.responses || {}).map(([key, val]) => (
                    <div key={key} className="p-3 text-xs flex justify-between">
            ...
            <div className="pt-4 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3 no-print">
              <button
                ...
                className="px-3.5 py-2 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold transition-colors flex items-center gap-1.5"
              >
              ...
              <button
                ...
                className="px-3.5 py-2 rounded-lg bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold transition-colors flex items-center gap-1.5"
              >
        ...
        {/* Right 4 Cols: Printable Ticket / Badge Preview (Prints directly!) */}
        <div className="lg:col-span-4 space-y-6">
          
          <div id="printable-ticket" className="bg-white rounded-2xl border-2 border-indigo-500/30 p-6 shadow-lg text-center relative overflow-hidden space-y-4">
            
            <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-indigo-600 via-cyan-400 to-indigo-600" />
            
            <div className="text-[10px] uppercase tracking-wider font-extrabold text-indigo-600 bg-indigo-50 py-1 px-3 rounded-full inline-block">
            ...
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 inline-block shadow-inner">
              {qrCodeUrl ? (
                <img src={qrCodeUrl} alt="Pass QR" className="w-40 h-40 mx-auto" />
              ) : (
            ...
            <div>
              ...
              <div className="text-sm font-extrabold text-indigo-700 mt-1">{reg.name}</div>
              <div className="text-[11px] text-slate-500 font-mono">{getPhoneDisplay(reg)}</div>
            </div>
            ...
            <div className="pt-2 no-print">
              <button
                ...
                className={`w-full py-2 px-3 rounded-lg text-xs font-bold shadow-sm transition-colors flex items-center justify-center gap-1.5 ${
                  reg.attendance_status === 'present'
              ...
              <form onSubmit={handleSaveEdit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-900 focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative flex items-center">
                    ...
                    <input
                      ...
                      className="w-full pl-16 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-900 font-mono focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                    />
                  </div>
                ...
                <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold transition-colors flex items-center gap-1.5 shadow-sm"
                  >
        ...
        {/* ========================================================================= */}
        {/* SEND / RESEND PASS MODAL (WhatsApp, Email & Direct Ticket Links)          */}
        {/* ========================================================================= */}
        {isSendModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs animate-in fade-in">
            <div className="bg-white rounded-2xl max-w-lg w-full border border-slate-200 shadow-2xl overflow-hidden relative max-h-[90vh] flex flex-col">
              ...
              <div className="p-5 space-y-4 overflow-y-auto flex-1 text-xs">
                ...
                <div className="p-4 rounded-xl border-2 border-emerald-500/20 bg-emerald-50/30 space-y-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-1">
                    <button
                      ...
                      className="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold transition-colors flex items-center gap-1.5 shadow-sm"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-2 rounded-xl bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-semibold transition-colors flex items-center gap-1.5"
                    >
                ...
                <div className="p-4 rounded-xl border border-slate-200 bg-white space-y-3">
                  ...
                  <p className="text-[11px] text-slate-600 leading-relaxed">
                    Opens your default email client (Gmail, Outlook, Apple Mail) with a pre-filled pass template:
                  </p>
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-1">
                    <button
                      ...
                      className="px-3.5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold transition-colors flex items-center gap-1.5 shadow-sm"
                    >
                      <Mail className="w-3.5 h-3.5" />
                    ...
                    <button
                      ...
                      className="px-3 py-2 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 font-semibold transition-colors flex items-center gap-1.5"
                    >
                      {copiedType === 'email' ? <Check className="w-3.5 h-3.5 text-indigo-600" /> : <Copy className="w-3.5 h-3.5" />}
                ...
                <div className="p-4 rounded-xl border border-slate-200 bg-slate-50/50 space-y-2">
                  ...
                  <div className="p-2 bg-white rounded-lg border border-slate-200 font-mono text-[11px] text-slate-600 truncate select-all">
                    {passUrl}
                  </div>
                </div>

                {/* Option 4: Automated Cloud Relay Simulation */}
                <div className="p-3.5 rounded-xl border border-indigo-100 bg-gradient-to-r from-indigo-50/60 to-blue-50/60 flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[10px] text-slate-500">
                      Dispatches pass to email and SMS via ACADENO webhook
                    </p>
                  </div>
